...
const regex = /<motion\.button[\s\S]*?id="bead-circle-counter"[\s\S]*?<\/motion\.button>/;
...
if (regex.test(content)) {
  content = content.replace(regex, newCounter);
  fs.writeFileSync('src/components/DailyView.tsx', content);
  console.log("Patched DailyView.tsx counter!");
} else {
